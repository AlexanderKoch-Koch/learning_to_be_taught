...
def build_and_train(slot_affinity_code=None, log_dir='./data', run_ID=0,
    ...
    episode_length = 40
    config = dict(
        vmpo_kwargs=dict(epochs=1, minibatches=5, discrete_actions=False, pop_art_reward_normalization=True,
                         T_target_steps=100),
        ppo_kwargs=dict(epochs=4, minibatches=1, learning_rate=1e-4),
        async_vmpo_kwargs=dict(epochs=4, discrete_actions=False, T_target_steps=40, batch_B=64,
                               batch_T=episode_length, pop_art_reward_normalization=True),
        sampler_kwargs=dict(batch_T=episode_length, batch_B=22 * 40,
                            env_kwargs=dict(id='Pendulum-v0'),
                            eval_env_kwargs=dict(id='Pendulum-v0', fixed_episode_length=None),
                            eval_n_envs=22,
                            eval_max_steps=1e5,
        ...
        runner_kwargs=dict(n_steps=1e10, log_interval_steps=2e6),
        snapshot=snapshot,
        # algo='vmpo',
        algo='async_vmpo'
    )
    ...
    if alternating_sampler:
        ...
        # AgentClass = CategoricalVmpoAgent
